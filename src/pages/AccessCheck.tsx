...
const errorMessage = (error: unknown) =>
  error instanceof Error ? error.message : 'Unable to complete the access check.';
const actionOptions = [
...
export default function AccessCheck() {
  ...
  useEffect(() => {
    const loadOptions = async () => {
      try {
        const [loadedUsers, loadedResources] = await Promise.all([
          getUsersApi(),
          getResourcesApi(),
        ]);
  ...
  const runCheck = async (event: React.FormEvent) => {
    ...
    try {
      setResult(
        await checkAuthorizationApi({ userId, resourceId, action, sessionDurationMinutes }),
      );
    } catch (requestError) {
      setError(errorMessage(requestError));
      setResult(null);
  ...
  return (
    <section className="page-container">
      <div className="page-header-row">
        <div>
          <h1 className="page-title">Access Check Simulator</h1>
          <p className="page-description">
            Replay an authorization decision before granting a privileged session.
          </p>
        </div>
      </div>
      {error && (
      ...
      <div className="simulator-layout">
        <form className="simulator-panel" onSubmit={runCheck}>
          ...
          {isLoading ? (
            <div className="simulator-loading" role="status">
              <div className="spinner" /> Loading users and resources...
            </div>
          ) : (
            <>
              ...
              <label className="form-group">
                <span className="form-label">Session duration (minutes)</span>
                <input
                  className="form-input"
                  type="number"
                  min="1"
              ...
              <button
                className="primary-action-btn simulator-submit"
                type="submit"
                disabled={isChecking || !userId || !resourceId}
              >
                {isChecking ? 'Checking...' : 'Evaluate access'}
              </button>
            </>
          )}
        </form>
        {result ? (
          <div
            className={`decision-panel decision-${result.decision.toLowerCase()}`}
            data-testid="decision-result"
          >
            <div className="decision-icon">{decisionIcon}</div>
            <span className="decision-label">{result.decision.replace('_', ' ')}</span>
            <h2>{result.reason}</h2>
          ...
          <div className="decision-placeholder">
            ...
            <p>Choose a user, resource, and action to see the authorization service decision.</p>
          </div>
        )}
      </div>
    </section>
